...
//	LOAD_Unused						= 0x00000800,
		...
		LOAD_DisableCompileOnLoad =
			0x00400000, ///< Prevent this load call from running compile on load for the loaded blueprint (intentionally not recursive, dependencies will still compile on load)
	};
 ...
 */
	public enum EPropertyFlags : ulong
	{
		...
		CPF_SkipSerialization =
			0x0080000000000000, ///< Property shouldn't be serialized, can still be exported to text
	};
 ...
 */
	...
	public enum EInternalObjectFlags
	{
		...
		AllFlags = ReachableInCluster | ClusterRoot | Native | Async | AsyncLoading | Unreachable | PendingKill |
		           RootSet
	};
...
// TODO: Move this to be contained in that module?
	public enum UM_UPROPERTY_MATERIAL
	{
...
// Metadata usable in UFUNCTION
	public enum UM_UFUNCTION
	{
		...
		/// [FunctionMetadata] For BlueprintCallable functions indicates that an input/output (determined by whether it is an input/output public enum) exec pin should be created for each entry in the public enum specified.
		ExpandEnumAsExecs,
...
-----------------------------------------------------------------------------*/
	 ...
	 */
	public enum ELoadConfigPropagationFlags
	{
		LCPF_None = 0x0,
